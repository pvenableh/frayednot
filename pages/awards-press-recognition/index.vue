<template>
  <div class="page recognition">
    ...
    <div class="page__body">
      ...
      <div
        class="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 my-12"
      >
        <a
          v-for="(article, index) in page.articles"
          :key="index"
          :href="article.link"
          target="_blank"
          class="press-card"
        >
          <div v-if="article.image && article.title" class="press-card__image">
            <img
              :src="imageUrl + article.image + '?key=small'"
              :alt="article.title"
            />
          </div>
      ...
      <div
        v-if="page.certified_intro"
        class="page__body-header flex items-center flex-col md:flex-row"
      >
        <div class="md:flex-grow-0">
          <h2 v-if="page.certified_intro.title" class="page__body-header-title">
            {{ page.certified_intro.title }}
          </h2>
          <div
            v-if="page.certified_intro.text"
            v-html="page.certified_intro.text"
            class="mb-4"
          ></div>
        </div>
        <img
          v-if="page.certified_intro.images.length"
          :src="
            imageUrl +
            page.certified_intro.images[0].directus_files_id +
            '?key=medium'
          "
          alt="HTA Logo"
      ...
      <div class="w-full">
        <div class="my-4"><p>Message from HTA president.</p></div>
        <video class="w-full" controls>
          <source
            src="https://admin.frayednot.net/assets/4c99ecc0-8819-4127-b07f-1e369a75ad9e"
            type="video/mp4"
          />
    ...
    <div
      v-if="page.recognition_intro"
      class="w-full shadow-inner my-20 recognition-intro"
    >
      <div class="page__body mx-auto">
        <div class="page__body-header">
          <h2
            v-if="page.recognition_intro.title"
            class="page__body-header-title"
          >
            {{ page.recognition_intro.title }}
          </h2>
          <h5 v-if="page.recognition_intro.subtitle">
            {{ page.recognition_intro.subtitle }}
          </h5>
          <div
            v-if="page.recognition_intro.text"
            class="relative"
            v-html="page.recognition_intro.text"
          ></div>
          <div
