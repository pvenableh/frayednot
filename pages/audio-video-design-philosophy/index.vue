<template>
  <div class="page philosophy">
    ...
    <div
      v-for="(section, index) in page.project_sections"
      :key="index"
      class="w-full page__body"
    >
      <div class="page__body-header">
        <h2
          v-if="section.project_sections_id.title"
          class="page__body-header-title"
        >
          {{ section.project_sections_id.title }}
        </h2>
        <h3 class="uppercase page__body-header-subtitle">
          {{ section.project_sections_id.sub_title }}
        </h3>
        <!-- <h5 v-if="section.project_sections_id.sub_title" class="uppercase -mt-8 mb-2 page__body-header-subtitle"></h5> -->
        <p v-if="section.project_sections_id.text" class="mb-12">
          {{ section.project_sections_id.text }}
        </p>
      </div>

      <div
        v-if="section.project_sections_id.layout === 'small'"
        class="grid sm:grid-cols-3 gap-2 sm:gap-4"
      >
        <ProjectsProjectCard
          v-for="(project, index) in section.project_sections_id.projects"
          :key="index"
          :project="project.projects_id"
          :size="section.project_sections_id.layout"
        />
      </div>
      <ProjectsProjectCard
        v-else
      ...
      <div class="w-full text-right mt-4">
        <UtilitiesLinkBtn link="/automated-audio-video-experiences"
          >Get Inspired
        </UtilitiesLinkBtn>
      </div>
    </div>
    <div class="page__body">
      <div class="page__body-header">
        <h2 v-if="technology.title" class="page__body-header-title">
          {{ technology.title }}
        </h2>
        <h3
          v-if="technology.subtitle"
        ...
        <div class="w-full text-right mt-4">
          <UtilitiesLinkBtn v-if="technology.link" :link="technology.link.link"
    ...
    <div
      v-if="page.certifications"
      class="page__body w-full flex flex-row items-center justify-between my-32 certifications"
    >
      <a
        :href="certification.link"
        target="_blank"
        v-for="(certification, index) in page.certifications"
        :key="index"
        class="flex items-center w-1/4 sm:w-1/5 md:w-1/6"
      >
        <img
          :src="imageUrl + certification.logo + '?key=medium'"
          :alt="certification.name"
          class="w-full h-auto"
        />
      </a>
      <a
        href="https://www.bbb.org/us/nj/kenilworth/profile/home-theater-systems/frayednot-never-left-stranded-0221-90189242/#sealclick"
        target="_blank"
        rel="nofollow"
        ><img
          src="https://seal-newjersey.bbb.org/seals/blue-seal-200-42-bbb-90189242.png"
          style="border: 0"
          alt="frayednot - never left stranded!® BBB Business Review"
      /></a>
    </div>
  </div>
</template>
